...
public class GridSchedulerNode implements IMessageReceivedHandler, Runnable {
    ...
    // Every GS node knows about the RMS connected to it by the GS frame
    //private ArrayList<String> connectedRMs = null;
    ...
    public GridSchedulerNode(String address){
        ...
        pollingThread.start();
        logger.warn("Supervisor " + this.address + " created.");
    ...
    // used for calculating the "load" of the gs nodes in the supervisor (grid scheduler) //TODO change this to reflect reality
    private int getNumberOfConnectedRMs(){
        return resourceManagerLoad.size();
        ...
        while (running) {
            ...
            for (Job job : jobQueue)
            {
                ...
                if (leastLoadedRM != null) {
                    ...
                    logger.info("[Supervisor] GS " + this.getAddress() +" sends a job to RM: " + leastLoadedRM);
